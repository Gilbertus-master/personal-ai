...
class SynergyResult(TypedDict):
    synergies_found: int
    total_bonus_pln: float
    details: list[dict]


def calculate_synergy(period_start: str, period_end: str) -> SynergyResult:
    """
    ...
    try:
        ...
        log.error("roi_synergy_db_error", error=str(exc))
        raise
